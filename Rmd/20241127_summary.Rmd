...
invoiced = 
  readxl::read_excel("invoiced.xlsx") %>%
...
For project types with $>1$ request.
...
ggplot(tally_river, aes(x=Department, 
    ...
    geom_text(aes(label=department_count), hjust=-0.3, size=3.5) +
    ylab("number of requests") +
    xlab("Department") +
    theme_classic() +
...
tally_inv = 
  ...
  group_by(Quarter) %>%
  summarize(Total = sum(Total.Cost),
            Project_Count = n())
```
