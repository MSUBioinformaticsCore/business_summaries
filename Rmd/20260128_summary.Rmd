...
project_cost = 
  invoiced %>%
  select(Jira, Total.Cost) %>%
  group_by(Jira) %>%
  summarise(Sum = sum(Total.Cost))
  
```

We have invoiced `r nrow(total_inv)` data analysis projects, totaling `r paste0("$", prettyNum(sum(invoiced$Total.Cost, na.rm = TRUE), big.mark = ",", digits=2))` with a median cost of `r paste0("$", prettyNum(median(project_cost$Sum, na.rm = TRUE), big.mark = ",", digits=2))` per project.
...
ggsave(p, file = "requests_per_quarter.png", width = 9, height = 5)
```
...
ggsave(p, file = "requests_per_quarter_by_type.png", width = 9, height = 5)
